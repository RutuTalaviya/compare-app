  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        // Fill up to 3 slots
        while (loadedSlots.length < 3) loadedSlots.push(null);
        setSlots(loadedSlots.slice(0, 3));
      }
    } catch (e) {
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-6">
        ...
        <div className="border border-[#d1d9e6] rounded-2xl bg-[#e6e7ee] shadow-soft overflow-hidden">
          ...
          <div className="p-6 md:p-8">
            <div className="flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-0 w-full relative">
              ...
              {slots.map((product, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="w-full lg:w-[30%] max-w-[420px] flex-shrink-0 flex items-center justify-center relative"
                  >
                    {/* Slot Card */}
                    <div
                    ...
                    >
                      {product ? (
                        <>
                          {/* Circular Score Rating Badge (Absolute top-left) */}
                          <div className="absolute top-4 left-4">
                            <CircularScore value={product.scoreValue} />
                          </div>

                          {/* Clear slot Button (Absolute top-right) */}
                          <button
                    ...
                    {/* VS separators (only display between slot cards) */}
                    {idx < 2 && (
                      <div className="absolute right-[-24px] lg:right-[-16px] xl:right-[-20px] top-1/2 -translate-y-1/2 lg:-translate-y-1/2 z-10 hidden lg:flex items-center justify-center">
